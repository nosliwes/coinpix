# this file runs model on a list of symbols 
...
# helper method to calculate EMA for model
def get_ema(df,span):
...
# helper method to calculate RSI for model
...
# method to prevent excessive trades
...
# method to run model and determine trading signals in trend
...
# plots buy and sell signals for visual confirmation
...
# get historical prices for a symbol from yahoo finance, default unless using robinhood or tda history
def get_historical(symbol, start_date, end_date):
    price_df = pdr.get_data_yahoo(symbol, start=start_date, end=end_date)
    return price_df

# method to backtest model signals against historical price data
...
# method to get historical prices from robinhood, must be logged in to robinhood account to use
...
# method to get historical prices from td ameritrade, must be logged in to tda account to use
...
# main method to run model on symbols
